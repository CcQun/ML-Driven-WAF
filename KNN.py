...
class WAF(object):
    ...
    def __init__(self):
        ...
        # 使用测试值 对 模型的准确度进行计算
        print('模型的准确度:{}'.format(self.knn.score(X_test[0:5000], y_test[0:5000])))
    ...
    def predict(self, new_queries):
        ...
        res = self.knn.predict(X_predict)
        res_list = []
...
if __name__ == '__main__':
    # w = WAF()
    # with open('Model/knn.pickle', 'wb') as output:
    #     pickle.dump(w, output)

    with open('Model/knn.pickle', 'rb') as input:
        w = pickle.load(input)
